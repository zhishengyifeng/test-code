...
static void shoot_delay_hanlder(trig_state_e *state, float shoot_delay)
{
	if (*state == TRIG_INIT)
	{
		...
		/* 卡弹角度锁定不正常则不自增角度 */
		if (single_shoot_angle - moto_trigger.total_angle <= 5 && single_shoot_angle - moto_trigger.total_angle >= -5)
		{
...
static void shoot_bullet_handler(void)
{
	...
	if (!global_err.list[JUDGE_SYS_OFFLINE].err_exist && (judge_recv_mesg.power_heat_data.shooter_id1_17mm_cooling_heat >= (judge_recv_mesg.game_robot_state.shooter_id1_17mm_cooling_limit - 10)))
