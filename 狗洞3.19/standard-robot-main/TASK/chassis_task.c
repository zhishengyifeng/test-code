...
float	power_new_pid[3] = {350.0f, 0.0f, 0.0f}; //新功率算法PID（代替下边的cap_vol_pid）
...
void chassis_param_init(void)
{
	...
	#else
  PID_Struct_Init(&pid_power, power_new_pid[0], power_new_pid[1], power_new_pid[2], 8000, 500, INIT);//新功率算法
	#endif
	/*底盘vx,vy平移的pid*/
	/////////////////
	for (int i = 0; i < 4; i++)
...
static void mecanum_calc(float vx, float vy, float vw, int16_t speed[]) // 底盘解算，得到底盘获得相应速度需要的四个电机值
{
	...
	int16_t wheel_rpm[4];
	float max = 0;
		if (INFANTRY_NUM == INFANTRY_5) // 步兵五即小全向的电机速度与普通麦轮步兵装配不同
