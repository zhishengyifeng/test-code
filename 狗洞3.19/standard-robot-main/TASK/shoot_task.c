...
static void shoot_bullet_handler(void)
{
  ...
  if (shoot.shoot_cmd)//单发
  {
		if(global_err.list[JUDGE_SYS_OFFLINE].err_exist == 1)//没连接裁判系统时
			...
			single_time = 1000/(judge_recv_mesg.game_robot_state.shooter_barrel_cooling_value/10 + judge_recv_mesg.game_robot_state.robot_level);
