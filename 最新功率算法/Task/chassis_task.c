...
#else
float chassis_vw_pid[3] = {1.0f, 0.01f, 0.0f};	   // 小陀螺转速控制PID
#endif
...
void chassis_power_contorl(pid_t *power_pid,float *power_vx,float *power_vy,float *power_yaw_speed,float real_time_Cap_remain,float real_time_Cap_can_store,float judge_power_limit)
{
    ...
    if (real_time_Cap_remain < 15.0f)
    {
        *power_yaw_speed = (5 - (15.0f - real_time_Cap_remain)) / 5 * 0.01f;//<0.01
    } // 低电容按比例减少鼠标左右移动速度
    else
    {
        *power_yaw_speed = 0.01f;
    ...
    if (real_time_Cap_remain > 18.0f && real_time_Cap_remain < real_time_Cap_can_store)//电容够电但没满电
    {
          min = 1350 + (cap_flag - (real_time_Cap_can_store - real_time_Cap_remain)) / cap_flag * (judge_power_limit - 45) / 45 * 500;
		}
    else if(real_time_Cap_remain <= 18.0f)//增加条件
    {
        min = 1150;//1000;
    }
		...
		if(km.vy != 0 || rm.vy != 0 || km.vx != 0 || rm.vx != 0)//底盘输入不为0,才进行pid运算（防止底盘静止时，pid->out积满）
		{
				if (Speed_up == 1)//不加速				
				{
					...
					{
						if(judge_recv_mesg.game_robot_state.chassis_power_limit - ob_total_power > Deta_Power)//功率相差过大
						{
							Ref_temp = ob_total_power + Deta_Power;							
							pid_calc(power_pid,ob_total_power,Ref_temp);
						}
						else
							pid_calc(power_pid,ob_total_power,judge_recv_mesg.game_robot_state.chassis_power_limit);
					}
					else//没接裁判系统
						pid_calc(power_pid,ob_total_power,Debug_Power+5);//没有连接裁判系统，小车底盘期望功率达到45w
				}
				else if (Speed_up == 0)//按下加速功率多100w
				{
					if(judge_recv_mesg.game_robot_state.chassis_power_limit>=45&&judge_recv_mesg.game_robot_state.chassis_power_limit<=220)
					{
						if(ob_total_power < 50)//防滑
							pid_calc(power_pid,ob_total_power,70);
						else
...
static void chassis_dodge_handler(void)
{
	...
	if (Speed_up == 1)//不加速				
	{
		if(judge_recv_mesg.game_robot_state.chassis_power_limit>=45&&judge_recv_mesg.game_robot_state.chassis_power_limit<=220)
		{
			pid_calc(&pid_chassis_vw,ob_total_power,judge_recv_mesg.game_robot_state.chassis_power_limit);
		}
		else//没接裁判系统
			pid_calc(&pid_chassis_vw,ob_total_power,Debug_Power+5);//没有连接裁判系统，小车底盘期望功率达到45w
	}
	else if (Speed_up == 0)//按下加速功率多100w
	{
		if(judge_recv_mesg.game_robot_state.chassis_power_limit>=45&&judge_recv_mesg.game_robot_state.chassis_power_limit<=220)
