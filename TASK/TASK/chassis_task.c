...
float chassis_pid_init[6] = {20, 0, 0, 50, 0, 0};
      ...
      if (Signal & INFO_GET_CHASSIS_SIGNAL)
      {
        ...
        if (chassis_mode != CHASSIS_RELEASE && gimbal.state != GIMBAL_INIT_NEVER) //云台归中之后底盘才能动
        {
          ...
          switch (chassis_mode)
          {
					...
					{
						for (int i = 0; i < 4; i++)
							PID_Struct_Init(&pid_spd[i], chassis_pid_init[0], chassis_pid_init[1], chassis_pid_init[2], 6000, 500,DONE);
						chassis_init_handler();
					}
          /*云台底盘跟随模式*/
          ...
          break;
					...
					if(chassis.state == CHASSIS_INIT_DONE)
						steering_calc(cx, chassis.vy, chassis.vw, chassis.wheel_ref);//ref:0、1 -- 3508, 2、3	--	6020
		
          for (int i = 0; i < 4; i++)//fdb -- 0、1 -- 3508, 2、3	--	6020
            chassis.current[i] = pid_calc(&pid_spd[i], (float)chassis.wheel_fdb[i], chassis.wheel_ref[i]);
...
static void chassis_init_handler()
{
	chassis.wheel_ref[2] = chassis.wheel_fdb[2] * ( 1 - ramp_calc(&pit_ramp));//利用斜坡函数使pitch轴的相对角度逐渐变为0
	chassis.wheel_ref[3] = chassis.wheel_fdb[3] * ( 1 - ramp_calc(&pit_ramp));
	if((chassis.wheel_fdb[2] <= 1 && chassis.wheel_fdb[2] >= -1) && (chassis.wheel_fdb[3] <= 1 && chassis.wheel_fdb[3] >= -1))
		chassis.state = CHASSIS_INIT_DONE;
}
...
static void chassis_stop_handler(void)
{
  for (int i = 0; i < 4; i++)
  {
    chassis.current[i] = pid_calc(&pid_spd[i], chassis.wheel_fdb[i], 0);
  }
...
void chassis_param_init(void)
{
  memset(&chassis, 0, sizeof(chassis));
	...
	chassis.state = CHASSIS_INIT_NEVER;
...
//	右轮:v1,id1 id5; 左轮:v2:id2 id6
//  input : ↑=+vx(mm/s)  ←=+vy(mm/s)  ccw=+vw(deg/s)
//  output : θ1、θ2，V1、V2 （θ是速度矢量与X轴的夹角）
static void steering_calc(float vx, float vy, float vw, float out[])
{
	static float rotate_ratio_l;//底盘半径(mm)
  static float wheel_rpm_ratio;

  taskENTER_CRITICAL();
  //@works
  rotate_ratio_l = WHEELRADIUS / RADIAN_COEF;
  wheel_rpm_ratio = 60.0f / (PERIMETER * CHASSIS_DECELE_RATIO);//  mm/s * wheel_rpm_ratio = rpm
  taskEXIT_CRITICAL();
  ...
  int16_t wheel_rpm[2];
  float max = 0;
	float theta1,theat2 = 0;
	float V1x,V1y,V2x,V2y = 0;
	
	//PS:vx vy 也许要反向
	V1x = vx + vw*rotate_ratio_l;//mm/s
	V1y = vy;
	V2x = vx - vw*rotate_ratio_l;
	V2y = vy;
	
	theta1 = atan(V1y/(V1x+0.000001))*RADIAN_COEF;//deg
	theat2 = atan(V2y/(V2x+0.000001))*RADIAN_COEF;
	
	if(V1x >= 0 )
	{
		wheel_rpm[0] = sqrt(V1x*V1x + V1y*V1y) * wheel_rpm_ratio;//rpm(内转子)
	}
	else if(V1x < 0 )
	{
		wheel_rpm[0] = -sqrt(V1x*V1x + V1y*V1y) * wheel_rpm_ratio;//rpm(内转子)
	...
	if(V2x >= 0 )
	{
		wheel_rpm[1] = -sqrt(V2x*V2x + V2y*V2y) * wheel_rpm_ratio;
	}
	else if(V2x < 0 )
	{
		wheel_rpm[1] = sqrt(V2x*V2x + V2y*V2y) * wheel_rpm_ratio;
  ...
  memcpy(out, wheel_rpm, 2 * sizeof(float));
	//6020
	out[2] = theta1;
	out[3] = theat2;
}
