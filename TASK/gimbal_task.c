...
// 普通参数（小陀螺与普通模式共用一套参数）
float pit_pid[6] = {55, 0.01, 30, 50, 0, 0};
float yaw_pid[6] = {50, 0, 10, 90, 0, 5};
...
float pit_init_pid[6] = {55, 0.015, 30, 70, 0, 0};
...
static void track_aimor_handler(void)
{
	...
	gimbal.pid.yaw_angle_fdb = gimbal.sensor.yaw_gyro_angle;  // yaw轴用陀螺仪
	 ...
	 LADRC_TD(&td_pit, MyCnt);//gimbal.sensor.pit_relative_angle + pc_recv_mesg.aim_pitch);
	 LADRC_TD(&td_yaw, gimbal.sensor.yaw_gyro_angle + pc_recv_mesg.aim_yaw);
...
if(a==1)
{
   work_time = HAL_GetTick();
	 MyCnt = 10;
	 a=2;
}
else if(a==2)
{
	 if(HAL_GetTick()-work_time>=2000)
	 {
		 MyCnt =0;
		 a=1;
	 }
...
//		pit_ctrl = td_pit.v1;
	...
	else if (pc_recv_mesg.mode_Union.info.visual_valid == 0)
	{
		if (last_vision_status == 1)
		{
			...
			lost_pit = gimbal.sensor.pit_relative_angle;					 //-pc_recv_result[1];//pit_msg_t.filtered_value;//丢失目标后云台不动
	...
	gimbal.pid.yaw_angle_ref = yaw_ctrl;
	gimbal.pid.pit_angle_ref = td_pit.v1;
...
static void shoot_buff_ctrl_handler(void)
{
	...
	else if (pc_recv_mesg.mode_Union.info.visual_valid == 0)
	{
		if (last_vision_status == 1)
		{
			lost_yaw = gimbal.sensor.yaw_gyro_angle + pc_recv_mesg.aim_yaw;
			lost_pit = gimbal.sensor.pit_relative_angle + pc_recv_mesg.aim_pitch; // 丢失目标后云台不动
			last_vision_status = 2;
		}
		yaw_ctrl = lost_yaw;
		pit_ctrl = lost_pit;
