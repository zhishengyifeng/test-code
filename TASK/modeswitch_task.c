...
void get_main_mode(void)
{
 ...
 else
 {
  switch(rc.sw2)
  {
    ...
    case RC_MI:
		{			
//      global_mode = MANUAL_CTRL;//FULL_AUTOMATIC_CTRL;//全自动
			if(TRACK_CTRL)//进入视觉     
        global_mode = SEMI_AUTOMATIC_CTRL;
      else
        global_mode = MANUAL_CTRL;
...
//				gimbal_mode = GIMBAL_SEPARATE_MODE;
    ...
    {		
			if(shoot.para_mode==SHOOTNOR_MODE)
        gimbal_mode = GIMBAL_TRACK_ARMOR; //云台自瞄模式
			//gimbal_mode = GIMBAL_SHOOT_BUFF;  //如需调试能量机关切换这段代码  
    }
    break;
...
//    case FULL_AUTOMATIC_CTRL:
//			if(last_gimbal_mode == GIMBAL_RELEASE | gimbal_mode == GIMBAL_RELEASE)
//			gimbal_mode = GIMBAL_INIT;
//			else if( gimbal.state == GIMBAL_INIT_DONE && RC_SEPARATE_MODE)
//      gimbal_mode = GIMBAL_SEPARATE_MODE; //云台底盘分离模式 
//			else if( gimbal.state == GIMBAL_INIT_DONE )
//			gimbal_mode = GIMBAL_NORMAL_MODE;
//    break;
