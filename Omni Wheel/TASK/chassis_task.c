...
float chassis_pid[3] = {20.0f, 0.0, 10.0f};
...
float	power_new_pid[3] = {10.0f, 4.0f, 0.0f}; //新功率算法PID（代替下边的cap_vol_pid）
...
void chassis_param_init(void)
{
	...
	#else
  PID_Struct_Init(&pid_power, power_new_pid[0], power_new_pid[1], power_new_pid[2], 4000, 3000, INIT);//新功率算法
	#endif
	/*底盘vx,vy平移的pid*/
	/////////////////
	for (int i = 0; i < 4; i++)
